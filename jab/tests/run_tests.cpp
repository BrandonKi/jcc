...
static Context
create_context(std::source_location loc = std::source_location::current()) {
    Context ctx;
...
static int run(Context &ctx, ModuleBuilder *builder) {
    if (jit_compile) {
        ...
        std::string cmd =
            ".\\" + ctx.options.output_dir + ctx.options.output_name;
#else
        std::string cmd =
            "./" + ctx.options.output_dir + ctx.options.output_name;
#endif
...
bool add() {
    Context ctx = create_context();
    auto *builder = ctx.new_module_builder(NAME);
    auto *add = builder->newFn("add", {Type::i64, Type::i64}, Type::i64,
                               CallConv::win64);
...
bool add4() {
    ...
    auto *builder = ctx.new_module_builder(NAME);
    auto *add =
        builder->newFn("add4", {Type::i64, Type::i64, Type::i64, Type::i64},
                       Type::i64, CallConv::win64);
    ...
    auto *jit = ctx.new_jit_env(builder);
    auto result =
        jit->run_function<i64 (*)(i64, i64, i64, i64)>("add4", 12, 8, 20, 15);
    return result == 55;
}
...
int main(int argc, char *argv[]) {
    ...
    //	link_coff_files("test",
    //{"C:/Users/Kirin/Desktop/just-another-backend/test.o"});
    //	link_coff_files("test",
    //{"C:/Users/Kirin/Desktop/just-another-backend/temp_files/test.obj"});
